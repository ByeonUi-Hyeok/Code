...
def removeTag( oriTxt ):           #함수선언

  ''' oriTxt : 특정 기호를 제거하고 싶은 문자열 // 여러줄 표시, 주석, 형식유지 할때 '''''' 사용한다'''



  return oriTxt.replace('&quot;','').replace('<b>','').replace('</b>','') #함수의 수행문
...
# 함수 호출
'''
#아마존이라는 키우드로 네이버에 뉴스검색 = > 결과를 간단한 정제후 리턴
#함수는, 여러곳에서 사용하거나 하나의 기능을 묶어버려서 사용 등등
# 함수는 call by value : 호출을 하면 값을 돌려준다.
#파이썬은 모든값이 객체이기 때문에 왼쪽값은 오른쪽 값을 가리킨다.
#res는 searchNewsByKeyword( '아마존' )가 리턴한 쪽을 가리킨다.(참조한다.)
 - [{키:값, ... }] - > dataFrame -> DB 테이블에 적제
'''
