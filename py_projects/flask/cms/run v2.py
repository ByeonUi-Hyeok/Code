...
app = Flask(__name__)
# 세션 관련 작업 - 세션키 저장
app.secret_key = 'hash value'          # 원래 해시값 사용.
         

@app.route('/')
def home() :
    # 세션 체크
    if not 'uid' in session :                       # 세션이라는 딕셔너리안에 uid라는 키가 존재하지않는가? (부정적 상황 체크)
...
def logout():
        if 'uid' in session :    # uid가 세션의 키로 존재한다면? ==> 세션이 있다.
            ...
            # 2. 홈페이지로 이동 ==> 세션이 없음 ==> 로그인화면(/login)으로 가게됨.
            return redirect('/')
...
def login() :
    ...
    if request.method == 'GET' :                  
        ...
        if row :     # row 는 None 이나 딕셔너리로 온다
            #4. 회원이면 == > 세션생성( 사용자의 어떤 정보를 저장 할 것인가== 여기서는 아이디만) ==> 
            session[ 'uid' ] = uid
            # 서비스 페이지 이동 ==> 리다이렉트 == 쓰려면 redirect 모듈 임포트해야함.
